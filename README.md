...
7. **Export** a pre-flight briefing as PDF, email, clipboard text, or KML. The KML export captures **every currently-visible map layer** as real geometry — airspace, TFRs, NOTAMs, obstacles, wires, airports, towers, fires, NWS alerts, ADS-B, etc. — with plain-text descriptions, category icons (plane for airports, circle-H for heliports), and NOTAM/TFR folders that lead with a plain-English summary before the raw text. It also adds **hourly sun & wind bearing lines** (24 h, time-slider animation; sun omitted while below the horizon, wind drawn downwind). Canopy and viewshed overlays export as **KMZ GroundOverlays** (the reliable georeferenced-image path for CalTopo and Google Earth) and, optionally, as **georeferenced GeoTIFFs** (EPSG:4326 + `.tfw`/`.prj` sidecars) for QGIS. Wire, obstacle, NOTAM and TFR folders carry a *"verify independently — not a complete inventory"* note
