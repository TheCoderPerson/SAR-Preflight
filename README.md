...
- **Active TFRs & NOTAMs**: auto-fetched live for your area on every draw via the built-in data proxy (see below) — TFRs plot + feed the assessment; NOTAMs plot + list in the NOTAMs tab (advisory — sourced from the unofficial FAA NOTAM Search backend; verify officially). If the proxy is unreachable it falls back to area-scoped deep-links + in-app file/paste import
