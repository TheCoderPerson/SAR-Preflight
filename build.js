...
const pattern = /<script src="sar-preflight-offline\.js"><\/script>\s*<script src="sar-preflight-core\.js"><\/script>\s*<script src="sar-preflight-charts\.js"><\/script>[\s\S]*?<script>\/\*.*?intentionally left empty.*?\*\/\s*<\/script>/;
...
// Copy service worker with build version stamp
const buildVersion = Date.now().toString(36);
let swCode = fs.readFileSync(path.join(dir, 'sw.js'), 'utf8');
swCode = swCode.replace("'__BUILD_VERSION__'", `'${buildVersion}'`);
// Strip CJS export from SW too
